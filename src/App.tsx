...
export default function App() {
  ...
  const [announcement, setAnnouncement] = useState<AnnouncementInfo>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('hp_announcement');
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch {
  ...
  const [bookings, setBookings] = useState<BookingReservation[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('hp_bookings');
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch {
  ...
  useEffect(() => {
    localStorage.setItem('hp_posts', JSON.stringify(posts));
  }, [posts]);

  useEffect(() => {
    localStorage.setItem('hp_lookbook', JSON.stringify(lookbookItems));
  }, [lookbookItems]);

  useEffect(() => {
    localStorage.setItem('hp_announcement', JSON.stringify(announcement));
  }, [announcement]);

  useEffect(() => {
    localStorage.setItem('hp_bookings', JSON.stringify(bookings));
  }, [bookings]);

  useEffect(() => {
    if (adminSession) {
      localStorage.setItem('hp_admin_session', JSON.stringify(adminSession));
    } else {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] dark:bg-[#121212] text-[#1D1D1D] dark:text-[#FAF9F6] transition-colors duration-300 font-sans">
      ...
      <LookbookMarquee items={lookbookItems} onSelectLook={handleSelectLook} />
